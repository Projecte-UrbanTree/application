...
export default function EditElementType() {
  ...
  useEffect(() => {
    const fetchElementType = async () => {
      try {
        const response = await api.get(`/admin/element-types/${id}`);
        const elementType = response.data;
        const responseIcons = await api.get('/admin/element-types/icons');
  ...
  useEffect(() => {
    const fetchIcons = async () => {
      try {
        const response = await api.get('/admin/element-types/icons');
  ...
  const handleSubmit = async (values: typeof initialValues) => {
    try {
      ...
      await api.put(`/admin/element-types/${id}`, updatedValues);
