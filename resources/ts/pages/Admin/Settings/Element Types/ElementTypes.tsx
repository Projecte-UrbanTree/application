...
export default function ElementsTypes() {
    ...
    const handleDelete = async (elementTypeId: number) => {
        if (!window.confirm(t('admin.pages.elementTypes.deleteConfirm')))
            return;
        try {
            await axiosClient.delete(`/admin/element-types/${elementTypeId}`);
            setElementTypes(
                elementTypes.filter(
                    (elementype) => elementype.id !== elementTypeId,
                ),
            );
        ...
        <>
            {msg && (
                <Message
                    severity={
                        successMsg ||
                        msg === t('admin.pages.elementTypes.deletedSuccess')
                            ? 'success'
                            : 'error'
                    }
                    text={msg}
            ...
            <CrudPanel
                ...
                onCreate={() =>
                    navigate('/admin/settings/element-types/create')
                }>
                <DataTable
                    value={elementTypes}
                    paginator
                        ...
                        body={(rowData: { id: number }) => (
                            <div className="flex justify-center gap-2">
                                <Button
                                    ...
                                    tooltip={t(
                                        'admin.pages.elementTypes.editButton',
                                    )}
                                ...
                                <Button
                                    ...
                                    tooltip={t(
                                        'admin.pages.elementTypes.deleteButton',
                                    )}
                                    tooltipOptions={{ position: 'top' }}
