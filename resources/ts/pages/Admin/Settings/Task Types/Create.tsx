  ...
  const handleSubmit = async (values: typeof initialValues) => {
    ...
    try {
      ...
      navigate("/admin/settings/task-types", {
        state: { success: t("admin.pages.taskTypes.success") }
      });
    } catch (error) {
      console.error(error);
      navigate("/admin/settings/task-types", {
          ...
          >
            {({ errors, touched }) => (
              <Form className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div className="flex flex-col">
                  <label className="flex items-center text-sm font-medium text-gray-700 mb-1">
                    {t("admin.pages.taskTypes.create.fields.description")}
                  </label>
                  <Field
                    name="description"
                    as={InputText}
                    placeholder={t("admin.pages.taskTypes.create.placeholders.description")}
