...
const EditWorkOrder = () => {
  ...
  useEffect(() => {
    const contractIdToUse =
      (!currentContract || currentContract === 0) && workOrderContract
        ? workOrderContract
        : currentContract;

    if (contractIdToUse) {
      api
        .get(`/admin/work-orders/create?contract_id=${contractIdToUse}`)
        .then((response) => {
          setUsers(
          ...
          setZones(
            response.data.zones.map((z: any) => ({ id: z.id, name: z.name })),
          );
  ...
  if (loading || !initialValues) return <Preloader bg_white={false} />;

  if (!currentContract && !workOrderContract)
    return <NoContractSelected returnButton="/admin/work-orders" />;
                        ...
                        <Card
                          ...
                          <div className="flex justify-between items-center mb-3">
                            <h4 className="text-lg font-bold flex items-center">
                              ...
                              />
                              {t('admin.pages.workOrders.form.fields.block')}{' '}
                              {index + 1}
                            </h4>
                          ...
                          <div className="flex flex-col mb-3">
                            ...
                            <MultiSelect
                              ...
                              options={zones}
                              onChange={(e) =>
                                setFieldValue(`blocks[${index}].zones`, e.value)
                              }
                              optionLabel="name"
                              placeholder={t(
