...
export const SensorList = () => {
  ...
  useEffect(() => {
    ...
    for (const [deviceName, sensorData] of Object.entries(groupedSensors)) {
      ...
      sensorData.variableReadings.sort((a, b) => 
        ...
        {data.allReadings.length > 1 ? (
          <div className="space-y-4">
            {data.allReadings.slice(1).map((reading: Reading, idx: number) => (
              <Card key={`${data.deviceName}-${reading.time}-${idx}`} className="bg-white shadow-sm">
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                  <div className="bg-gray-50 p-3 rounded-lg border border-gray-200">
                    <p className="text-xs text-gray-500 mb-1">{t('sensors.monitoring.labels.temp_soil')}</p>
                    <div className="flex items-center">
                      <Icon icon="tabler:temperature" className="text-red-500 mr-2" />
                      <span className="text-gray-700">{formatValue(reading.temp_soil, '°C')}</span>
                    </div>
                  </div>
                  
                  <div className="bg-gray-50 p-3 rounded-lg border border-gray-200">
                    <p className="text-xs text-gray-500 mb-1">{t('sensors.monitoring.labels.ph_soil')}</p>
                    <div className="flex items-center">
                      <Icon icon="tabler:flask" className="text-purple-500 mr-2" />
                      <span className="text-gray-700">{formatValue(reading.ph1_soil)}</span>
                    </div>
                  </div>
                  
                  <div className="bg-gray-50 p-3 rounded-lg border border-gray-200">
                    <p className="text-xs text-gray-500 mb-1">{t('sensors.monitoring.labels.battery')}</p>
                    <div className="flex items-center">
                      <Icon icon="tabler:battery" className="text-green-500 mr-2" />
                      <span className="text-gray-700">{formatValue(reading.bat, 'V')}</span>
                    </div>
                  </div>
                  
                  <div className="bg-gray-50 p-3 rounded-lg border border-gray-200">
                    <p className="text-xs text-gray-500 mb-1">{t('sensors.monitoring.labels.signal')}</p>
                    <div className="flex items-center">
                      <Icon icon="tabler:signal" className="text-blue-500 mr-2" />
                      <span className="text-gray-700">
                        {reading.rssi ? `${reading.rssi} dBm` : '-'} / {reading.snr ? `SNR: ${reading.snr}` : '-'}
                      </span>
                    </div>
                  </div>
                </div>
    ...
    <div className="container mx-auto px-4 py-8">
      <div className="flex justify-between items-center mb-6">
        <div className="flex items-center gap-3">
          ...
          <h1 className="text-2xl font-bold text-gray-800">{t('sensors.monitoring.title')}</h1>
        </div>
      </div>
      ...
      {groupedSensorsList.length === 0 ? (
        <Card className="border border-blue-300 bg-blue-50">
          <div className="flex flex-col items-center gap-3 py-8">
            ...
            <h3 className="text-center text-blue-700 font-medium">{t('sensors.monitoring.noSensors')}</h3>
          </div>
        </Card>
      ) : (
        <DataTable 
        ...
        >
          ...
          <Column field="deviceName" header={t('admin.pages.sensors.fields.device_name')} body={nameTemplate} sortable />
          <Column field="dev_eui" header={t('admin.pages.sensors.fields.dev_eui')} sortable />
          <Column header={t('sensors.monitoring.lastUpdateLabel')} body={latestTimeTemplate} sortable field="time" />
          <Column header={t('sensors.monitoring.labels.battery')} body={batteryTemplate} sortable field="bat" />
          <Column header={t('sensors.monitoring.labels.signal')} body={signalTemplate} sortable field="rssi" />
          <Column header={t('sensors.monitoring.labels.temp_soil')} body={temperatureTemplate} sortable field="temp_soil" />
          <Column header={t('sensors.monitoring.labels.water_soil')} body={moistureTemplate} sortable field="water_soil" />
          <Column header={t('sensors.monitoring.locationTitle')} body={locationTemplate} />
        </DataTable>
      )}
    </div>
  );
